...
public class GSSReceiver : MonoBehaviour
{
    ...
    public void OnGSSLoadEnd()
    {
        ...
        if (sheetDataslist != null)
        {
            ...
            DataBaseManager.instance.deckDataSO.deckDatasList =
                new List<DeckData>(sheetDataslist.Find(x => x.SheetName == SheetName.DeckData).DatasList.Select(x => new DeckData(x)).ToList());

            // スクリプタブル・オブジェクトに代入(例２)
            if (DataBaseManager.instance.cardDataSO.cardDatasList != null)
            {
